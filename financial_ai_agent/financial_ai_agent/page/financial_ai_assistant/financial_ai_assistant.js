...
class FinancialAIDeskPage {
  ...
  async connector_changed() {
    ...
    if(connector.crm_type==="Salesforce Headless 360") { button.text(__("Checking Salesforce…")).prop("disabled",true); try { const {message}=await frappe.call("financial_ai_agent.api.crm.salesforce_status",{connector:connector.name}); this.salesforce=message; button.prop("disabled",false).text(message.connected?__("Disconnect Salesforce"):__("Connect Salesforce")); this.root.find("#fai-crm-dot").toggleClass("connected",Boolean(message.connected)); detail.append(`<span>${this.escape(message.connected?(message.salesforce_username||message.instance_url||__("OAuth connected")):(message.message||__("OAuth connection required")))}</span>`); } catch(error){button.prop("disabled",false).text(__("Connect Salesforce"));this.log(__("Salesforce status failed"),"error",this.error_text(error));} }
  ...
  async connector_action() {
    ...
    if(connector.crm_type==="Salesforce Headless 360") { if(this.salesforce?.connected){frappe.confirm(__("Disconnect your Salesforce account?"),async()=>{await frappe.call("financial_ai_agent.api.crm.disconnect_salesforce",{connector:connector.name});this.log(__("Salesforce disconnected"),"info");this.connector_changed();});return;} try { const {message}=await frappe.call("financial_ai_agent.api.crm.start_salesforce_oauth",{connector:connector.name}); this.log(__("Starting Salesforce OAuth"),"pending"); window.location.assign(message.authorization_url); } catch(error){this.notice(this.error_text(error));this.log(__("Salesforce connection failed"),"error",this.error_text(error));} return; }
